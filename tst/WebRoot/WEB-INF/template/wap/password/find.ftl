...
<script type="text/javascript" src="${base}/statics/js/jquery.cookie.js?v=2.6.0.161014"></script>
	<div class="mui-content">
		<div class="padding bg-white password-find choose-way">
			...
			<a class="mui-btn full mui-btn-blue margin-top line78" id="btn-way">确认选择方式</a>
		</div>
		...
		<div class="padding bg-white password-find email-dom">
			...
			<a id="send_email" class="mui-btn full mui-btn-blue margin-top line78">发送电子邮件</a>
		</div>
		...
		<div class="padding bg-white mobile-dom password-find">
			...
			<div class="list relative">
	        	<input type="text" style="font-size: .28rem;" class="number" placeholder="请输入手机验证码" name="vcode" id="vcode" disabled="disabled"  ajaxurl="{url('ajax_register_vcode_check')}"/>
	        	<span class="validform_checktip"></span>
	        	<button class="mui-btn mui-btn-primary ver-code" id="send_sms">发送验证码</button>
	        </div>
			<a class="mui-btn full mui-btn-blue line78" id="next_btn">下一步</a>
		</div>
	</div>
...
<script>
	...
	$("#next_btn").on('tap',function(){
		if(1 || $mobile.data("verify") && vocde){
			...
			$.ajax({
                type: 'POST',
                dataType: 'json',
                url: '/wap/password/reset_sms.jhtml',
                data: data,
    ...
    function send_sms(mobile) {
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: '${base}/wap/password/send_sms.jhtml',
            data: {mobile: mobile},
	...
	function verify_ajax(data){
		...
		$.ajax({
			type:"post",
            dataType: 'json',
			url:'${base}/wap/password/send_email.jhtml',
			data:data,
			success:function(result){
				if(result.status == 1){
