...
<script type="text/javascript" src="${base}/statics/js/member.order.js"></script>
		<div class="hd-grid filter-items bg-white">
			<div class="hd-col-xs-e5">
				<a href="${base}/wap/member/order/list.jhtml" class="filter-item [#if status == 'all'] [#if isdiz != '1']current[/#if][/#if]">全部</a>
			</div>
			<div class="hd-col-xs-e5">
				<a href="${base}/wap/member/order/list.jhtml?isdiz=1" class="filter-item [#if isdiz == '1'] current[/#if]">订制</a>
			</div>
			...
			<div class="hd-col-xs-e5">
				<a href="${base}/wap/member/order/list.jhtml?status=shipped" class="filter-item [#if status == 'shipped'] current[/#if]">待签收</a>
			</div>
			<div class="hd-col-xs-e5">
				<a href="${base}/wap/member/order/list.jhtml?status=completed" class="filter-item [#if status == 'completed'] current[/#if]">已完成</a>
			</div>
		</div>
		<div id="refreshContainer" class="mui-content" style="padding-top: 1.45rem;">
				...
				[#if pages?? && pages.list?has_content]
					[#list pages.list as order]
						<li class="order-list list-col-10">
							...
							<!-- 循环子订单 -->
							<div class="padding-tb mui-text-left list-item order-stuats ">
								<span class="icon icon-15 margin-big-right"><img src="${base}/statics/images/ico_17_1.png"></span>
								<span class="margin-big-right">共 ${order.quantity} 件商品</span>
								<span class="mui-pull-right">订单总额：<b class="text-org">${currency(order.amount, true, true)}</b></span>
							</div>
...
[#if isdiz??&&isdiz=="1"]
<a href="${base}/wap/member/order/zhuisu.jhtml?sn=${order.sn}" class="mui-btn hd-btn-gray">产品追溯
[/#if]
				                ...
				                [#if order.expire?? && order.hasExpired() || order.statusName == 'completed' || order.statusName == 'received' ||  order.statusName == 'canceled' || order.statusName == 'failed' || order.statusName == 'denied']
				                	[#list order.orderItems as orderItem]
				                	<a data-action="againss" href="/wap/goods/detail.jhtml?id=${orderItem.product.goods.id}" class="mui-btn hd-btn-gray">
				                	再次购买</a>
				                	[/#list]
				                [/#if]
