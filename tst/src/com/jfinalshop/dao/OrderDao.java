...
public class OrderDao extends BaseDao<Order> {
	...
	public List<Order> findList(Order.Type type, Order.Status status,
			Member member, Goods goods, Boolean isPendingReceive,
			Boolean isPendingRefunds, Boolean isUseCouponCode,
			Boolean isExchangePoint, Boolean isAllocatedStock,
			Boolean hasExpired, Integer count, List<Filter> filters,
		...
		if (goods != null) {
			sql += " AND EXISTS (SELECT 1 FROM `order_item` oi WHERE o.id = oi.order_id AND oi.product_id IN (SELECT p.id FROM `product` p LEFT JOIN `goods` g ON p.goods_id = g.id AND g.id = "
					+ goods.getId() + ")) ";
		}
		if (isPendingReceive != null) {
			String subQuery = "" + " ((o.expire IS NULL OR o.expire > '"
					+ DateUtils.formatDateTime(new Date())
					+ "') AND o.payment_method_name = "
					+ PaymentMethod.Type.cashOnDelivery.ordinal()
		...
		if (hasExpired != null) {
			if (hasExpired) {
				sql += " AND o.expire IS NOT NULL AND o.expire <= '"
						+ DateUtils.formatDateTime(new Date()) + "' ";
			} else {
				sql += " AND o.expire IS NULL OR o.expire > '"
						+ DateUtils.formatDateTime(new Date()) + "' ";
			}
		}
	 ...
	 * @return 订单分页
	 */
	public Page<Order> findPage(Order.Type type, Order.Status status,
			Member member, Goods goods, Boolean isPendingReceive,
			Boolean isPendingRefunds, Boolean isUseCouponCode,
			Boolean isExchangePoint, Boolean isAllocatedStock,
			Boolean hasExpired, Pageable pageable,String isdz) {
		...
		if (goods != null) {
			sqlExceptSelect += " AND EXISTS (SELECT 1 FROM `order_item` oi WHERE o.id = oi.order_id AND oi.product_id IN (SELECT p.id FROM `product` p LEFT JOIN `goods` g ON p.goods_id = g.id AND g.id = "
					+ goods.getId() + ")) ";
		}
		...
		if (isdz != null) { //是否定制
			sqlExceptSelect += " AND EXISTS (SELECT 1 FROM `order_item` oi WHERE o.id = oi.order_id AND oi.product_id IN (SELECT p.id FROM `product` p ,`goods` g where p.goods_id = g.id AND g.is_dz ='"+isdz+"')) ";
		}
		...
		if (hasExpired != null) {
			if (hasExpired) {
				sqlExceptSelect += " AND o.expire IS NOT NULL AND o.expire <= '"
						+ DateUtils.formatDateTime(new Date()) + "' ";
			} else {
				sqlExceptSelect += " AND o.expire IS NULL OR o.expire > '"
						+ DateUtils.formatDateTime(new Date()) + "' ";
			}
		}
	 ...
	 * @return 订单数量
	 */
	public Long count(Order.Type type, Order.Status status, Member member,
			Goods goods, Boolean isPendingReceive, Boolean isPendingRefunds,
			Boolean isUseCouponCode, Boolean isExchangePoint,
			Boolean isAllocatedStock, Boolean hasExpired) {
	...
	public Long createOrderCount(Date beginDate, Date endDate) {
		...
		if (beginDate != null) {
			sqlExceptSelect += " AND create_date >= '"
					+ DateUtils.formatDateTime(beginDate) + "' ";
		}
		if (endDate != null) {
			sqlExceptSelect += " AND create_date <= '"
					+ DateUtils.formatDateTime(endDate) + "' ";
		}
	...
	public Long completeOrderCount(Date beginDate, Date endDate) {
		String sqlExceptSelect = "FROM `order`  WHERE 1 = 1 ";
		if (beginDate != null) {
			sqlExceptSelect += " AND complete_date >= '"
					+ DateUtils.formatDateTime(beginDate) + "' ";
		}
		if (endDate != null) {
			sqlExceptSelect += " AND complete_date <= '"
