...
public class MessageDao extends BaseDao<Message> {
	...
	public Page<Message> findPage(Member member, Pageable pageable, Boolean read) {
		...
		if (member != null) {
			sqlExceptSelect += "AND ((sender_id = " + member.getId()
					+ " AND sender_delete = false ) OR (receiver_id = "
					+ member.getId() + " AND receiver_delete = false)) ";
		} else {
	...
	public Long count(Member member, Boolean read) {
		...
		if (member != null) {
			if (read != null) {
				sqlExceptSelect += "AND ((sender_id = " + member.getId()
						+ " AND sender_delete = false AND sender_read = "
						+ read + ") OR (receiver_id = " + member.getId()
						+ " AND receiver_delete = false AND receiver_read = "
						+ read + ")) ";
			} else {
				sqlExceptSelect += "AND ((sender_id = " + member.getId()
						+ " AND sender_delete = false) OR (receiver_id = "
			...
			if (read != null) {
				sqlExceptSelect += "AND ((sender_id IS NULL OR sender_delete = false OR sender_read = "
						+ read
						+ ") OR (receiver_id IS NULL OR receiver_delete = false OR receiver_read = "
						+ read + ")) ";
			} else {
				sqlExceptSelect += "AND ((sender_id IS NULL OR sender_delete = false) OR (receiver_id IS NULL OR receiver_delete = false))";
			}
	...
	public void remove(Long id, Member member) {
		...
		if ((member != null && member.equals(message.getReceiver()))
				|| (member == null && message.getReceiver() == null)) {
		...
		} else if ((member != null && member.equals(message.getSender()))
				|| (member == null && message.getSender() == null)) {
			if (message.getIsDraft()) {
				super.remove(message);
			} else {
