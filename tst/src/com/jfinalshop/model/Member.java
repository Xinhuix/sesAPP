...
public class Member extends BaseMember<Member> {
    ...
    //public static final SCardorder daos=new SCardorder();
	...
	public Gender getGenderName() {
		return Member.Gender.values()[getGender()==null?0:1];
	}
	...
	/*** 会员月卡记录 ***/
	private SCardorder scardorder;
	

	public SCardorder getScardorder() {
		if (ObjectUtils.isEmpty(scardorder)) {
			String sql = "SELECT * FROM s_cardorder WHERE status='1' and member_id = ? order by end_date";
			scardorder = SCardorder.dao.findFirst(sql, getId());
		}
		return scardorder;
	}
	...
	public void setAttributeValue(MemberAttribute memberAttribute, Object memberAttributeValue) {
		...
		case gender:
			if (memberAttributeValue instanceof Member.Gender || memberAttributeValue == null) {
				...
				setGender(gender==null?0:gender.ordinal());
			}
			break;
		case birth:
