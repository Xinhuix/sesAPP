...
public class OrderItem extends BaseOrderItem<OrderItem> {
	...
	public BigDecimal getSubtotal() {
		System.out.println("这是小计:"+getPrice()+ "这是数量:"+getQuantity());
	 ...
	 * @throws Exception
	 */
	public void Receipt(Order order) throws Exception {
		System.out.println(order);
		if(order==null) {
			throw new Exception("订单不能为空哦");
		}	
		List<OrderItem> orderItems= find("select * from order_item where order_id=?",order.getId());
		...
		if(orderItems.size()!=0) {
			...
			Date date = new Date();
		Db.update("UPDATE order_item SET modify_date=?,version=?,order_item_status=4 WHERE id in("+StringUtils.strip(list.toString(),"[]")+") ",date,orderItems.get(0).getVersion()+1);
		}
	}
