...
public class SmsService extends BaseService<Sms> {
	...
	private SmsDao smsDao = Enhancer.enhance(SmsDao.class);
	...
	private void send(String[] mobiles, String content, Date sendTime) {
		...
		LogKit.info("发送短信验证码...");
		String mobiles_str=StringUtils.join(mobiles, ",");
//System.out.println("mobiles_str--->"+mobiles_str);
...
//			}
			...
			HttpClient httpclient= new com.jfinalshop.util.SSLClient();
			String url = "https://dx.ipyy.net/sms.aspx";
			String accountName=smsSn;							//改为实际账号名
			String password=smsKey;								//改为实际发送密码
			String text ="";
			 ...
			 Pattern pattern = Pattern.compile("[0-9]*");
	           ...
	           if(isNum.matches() ){
	        	 text = "您的短信验证码："+content+"【农业4.0先驱->易视购】";
	           }else{
	   			text = content+"【易视购】";  
	           }
			...
			nvps.add(new BasicNameValuePair("mobile", mobiles_str));		//多个手机号用逗号分隔
...
//			}
	...
	public void saveOrUpdate(Sms sms) {
		Sms pSms = smsDao.find(sms.getMobile(), null, sms.getSmsType());
		if(pSms == null) {
			super.save(sms);
		} else {
			pSms.setSmsCode(sms.getSmsCode());
			super.update(pSms);
		}
	}
