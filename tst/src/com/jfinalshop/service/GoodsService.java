...
public class GoodsService extends BaseService<Goods> {
	...
	private ProductCategoryDao productCategoryDao = Enhancer
			.enhance(ProductCategoryDao.class);
	...
	// Enhancer.enhance(SpecificationValueService.class);
	private ProductImageService productImageService = Enhancer
			.enhance(ProductImageService.class);
	...
	public List<Goods> findList(Goods.Type type,
		...
		return goodsDao.findList(type, productCategory, brand, promotion, tag,
				...
				count, filters, orders,null);
	}
 /**
  * @author zxx
  */
	public List<Goods> clientFindList(Goods.Type type, Long productCategoryId,
			...
			List<Filter> filters, List<Order> orders, boolean useCache,String is_dz) {
	...
	public Goods save(Goods goods, List<Product> products, Admin operator) {
		...
		// final List<SpecificationItem> specificationItems =
		// goods.getSpecificationItemConverter();
	...
	public Goods update(Goods goods, Product product, Admin operator) {
		...
		copyProperties(goods, pGoods, "sn", "type", "score", "totalScore",
				...
				"consultations", "favoriteMembers", "products", "createDate");
		...
		if (CollectionUtils.exists(products, new Predicate() {
			...
			public boolean evaluate(Object object) {
				...
				return product == null || !product.isNew()
						|| !product.hasSpecification()
						|| !set.add(product.getSpecificationValueIds()); // ) ||
																			// !specificationValueService.isValid(specificationItems,
	...
	private void setValue(Goods goods) {
		...
		List<ProductImage> productImagesConverter = goods
				.getProductImagesConverter();
