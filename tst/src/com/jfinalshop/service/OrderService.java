...
public class OrderService extends BaseService<Order> {
	 ...
	 * @return 订单分页
	 */
	public Page<Order> findPage(Order.Type type, Order.Status status, Member member, Goods goods, Boolean isPendingReceive, Boolean isPendingRefunds, Boolean isUseCouponCode, Boolean isExchangePoint, Boolean isAllocatedStock, Boolean hasExpired, Pageable pageable,String isdz) {
		return orderDao.findPage(type, status, member, goods, isPendingReceive, isPendingRefunds, isUseCouponCode, isExchangePoint, isAllocatedStock, hasExpired, pageable,isdz);
	}
...
//				orderDao.clear();
	...
	public void returns(Order order, Returns returns, Admin operator) {
		...
		for (ReturnsItem returnsItem : returns.getReturnsItems()) {
			...
			if(operator==null) {
				returnsItem.setCause("用户申请退单操作");
			}else {
				returnsItem.setCause(operator.getName()+"退单操作");
			}
			...
			orderItem.setOrderItemStatus(6);
	...
	public void complete(Order order, Admin operator) {
		...
		if (order.getRewardPoint() > 0) { //增加积分
		...
		if (order.getAmountPaid().compareTo(BigDecimal.ZERO) > 0) { //增加消费金额
