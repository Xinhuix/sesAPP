...
public class OrderController extends BaseController {
	...
	public void calculate() {
		...
		data.put("amount", orderService.calculateAmount(order.getPrice(),
				order.getFee(), freight, tax, order.getPromotionDiscount(),
				order.getCouponDiscount(), offsetAmount));
		renderJson(data);
	}
	...
	public void transitStep() {
		...
		if (StringUtils.isEmpty(setting.getKuaidi100Key())
				|| StringUtils.isEmpty(shipping.getDeliveryCorpCode())
				|| StringUtils.isEmpty(shipping.getTrackingNo())) {
			data.put("message", ERROR_MESSAGE);
			renderJson(data);
			return;
		}
	...
	public void orderreview() {
		...
		setAttr("order", order);
		render("/admin/order/orderreview.ftl");
	}
	
	/**
	 * 订单追溯 提交
	 */
	public void orderreviewSubmint() {
		Long id = getParaToLong("id");
		Order order = orderService.find(id);
		String review = getPara("review");// 订单追溯
		order.setReview(review);// 订单追溯
	...
	public void update() {
		...
		if (order == null
				|| order.hasExpired()
				|| (!Order.Status.pendingPayment.equals(order.getStatusName()) && !Order.Status.pendingReview
						.equals(order.getStatusName()))) {
			redirect("ERROR_VIEW");
		}
		Invoice invoice = StringUtils.isNotEmpty(invoiceTitle) ? new Invoice(
	...
	public void review() {
		...
		if (order == null || order.hasExpired()
				|| !Order.Status.pendingReview.equals(order.getStatusName())) {
	...
	public void payment() {
		...
		Payment.Method method = StrKit.notBlank(methodName) ? Payment.Method
		...
		if (Payment.Method.deposit.equals(payment.getMethod())
	...
	public void shipping() {
		...
		List<ShippingItem> shippingItems = getBeans(ShippingItem.class,
				"shippingItems");
		...
		for (Iterator<ShippingItem> iterator = shipping.getShippingItems()
			...
			if (orderItem == null
					|| shippingItem.getQuantity() > orderItem
			...
			if (product != null
					&& shippingItem.getQuantity() > product.getStock()) {
				redirect(ERROR_VIEW);
				return;
			}
	...
	public void returns() {
		...
		List<ReturnsItem> returnsItems = getBeans(ReturnsItem.class,
				"returnsItems");
		
		returns.setReturnsItems(returnsItems);

		for (Iterator<ReturnsItem> iterator = returns.getReturnsItems()
				.iterator(); iterator.hasNext();) {
			...
			if (orderItem == null
					|| returnsItem.getQuantity() > orderItem
	...
	public void receive() {
		...
		Order order = orderService.find(id);
		if (order == null || order.hasExpired()
				|| !Order.Status.shipped.equals(order.getStatusName())) {
			redirect(ERROR_VIEW);
			return;
		}
		OrderItem orderItem = new OrderItem();
		try {
			orderItem.Receipt(order);
		} catch (Exception e) {
			e.printStackTrace();
		}
		Admin admin = adminService.getCurrent();
	...
	public void complete() {
		...
		Order order = orderService.find(id);
		if (order == null || order.hasExpired()
				|| !Order.Status.received.equals(order.getStatusName())) {
			redirect(ERROR_VIEW);
			return;
		}
		Admin admin = adminService.getCurrent();
	...
	public void fail() {
		...
		if (order == null
				|| order.hasExpired()
				|| (!Order.Status.pendingShipment.equals(order.getStatusName())
						&& !Order.Status.shipped.equals(order.getStatusName()) && !Order.Status.received
							.equals(order.getStatusName()))) {
			redirect(ERROR_VIEW);
			return;
		}
	...
	public void list() {
		...
		Order.Type type = StrKit.notBlank(typeName) ? Order.Type
		...
		Order.Status status = StrKit.notBlank(statusName) ? Order.Status
				.valueOf(statusName) : null;
		...
		Boolean isdz_false = getParaToBoolean("isdz_false");//现货
		Boolean isdz_true = getParaToBoolean("isdz_true");//订制
		
		String isdz=null;
	
		if(isdz_false!=null&&isdz_false){
			isdz="0";
		}else if(isdz_true!=null&&isdz_true){
			isdz="1";
		}
		...
		if (StringUtils.isNotEmpty(memberUsername) && member == null) {
			...
			setAttr("page", orderService.findPage(type, status, member, null,
					isPendingReceive, isPendingRefunds, null, null,
					isAllocatedStock, hasExpired, pageable,isdz));
		}
